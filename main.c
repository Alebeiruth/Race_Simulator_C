...
const char *printIntro() {
  printf("Welcome to our main event digital race fans!I hope everybody has "
         "their snacks because we are about to begin!");
}
...
void printCongratulation(struct Race race) {
  printf(
      "Let's all congratulate Cosmo in the orange race car for an amazing "
      "performance.It truly was a great race and everybody have a goodnight!");
}
...
void updateFirstPlace(struct Race *racePointer, struct RaceCar *raceCar1Pointer,
                      struct RaceCar *raceCar2Pointer) {
...
void startRace(struct RaceCar *raceCar1Pointer,
               struct RaceCar *raceCar2Pointer) {
...
int main() {
  ...
  struct RaceCar raceCar1 = {
      .driverName = "Senna", .raceCarColor = "Red", .totalLapTime = 0};
  struct RaceCar raceCar2 = {
      .driverName = "Shummy", .raceCarColor = "White", .totalLapTime = 0};
  printIntro();
